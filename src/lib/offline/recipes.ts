...
/**
 * Ask the browser to fetch (and let the service worker cache) the React Server
 * Component payload for each recipe detail route.
 *
 * Caching the data locally isn't enough on its own: navigating to
 * /recipes/<id> offline still needs *something* cached for that URL or the
 * router has nothing to render. The detail route is a data-free shell, so each
 * payload is tiny. Already-cached routes are skipped, and failures are ignored.
 */
export async function warmRecipeRoutes(ids: string[]): Promise<void> {
  if (typeof caches === "undefined" || !navigator.onLine) return;

  // A fixed marker query keeps the warmed payload in its own cache entry, so it
  // can never be mistaken for the route's cached HTML document.
  const pending: string[] = [];
  for (const id of ids) {
    const url = `/recipes/${id}?_rsc=warm`;
    try {
      const hit = await caches.match(url, { ignoreVary: true });
      if (!hit) pending.push(url);
    } catch {
      return; // Cache API unavailable — nothing to warm
    }
  }

  // Small concurrency so warming never competes with what the user is doing.
  const queue = [...pending];
  const workers = Array.from({ length: 3 }, async () => {
    for (let url = queue.shift(); url; url = queue.shift()) {
      try {
        ...
        return; // went offline — stop quietly
      }
    }
  });
